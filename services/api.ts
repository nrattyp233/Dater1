...
export const getMatches = async (currentUserId: number): Promise<number[]> => {
  // Get users the current user has swiped right on who also swiped right on them
  const { data, error } = await supabase.rpc('get_matches', { current_user_id: currentUserId });
...
export const expressPriorityInterest = async (userId: number, dateId: number): Promise<DatePost> => {
  // First, add user to applicants if not already there
  await supabase.rpc('add_applicant_to_date', {
    p_date_id: dateId,
    p_user_id: userId
  });
  
  // Then add to priority applicants
...
export const recordSuperLike = async (userId: number, swipedUserId: number): Promise<{ isMatch: boolean }> => {
  // Record a super like (treated as a special type of right swipe)
...
export const recallSwipe = async (userId: number, lastSwipedUserId: number): Promise<void> => {
  const { error } = await supabase
    ...
    .delete()
    .eq('user_id', userId)
    .eq('target_user_id', lastSwipedUserId)
