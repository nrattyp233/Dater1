-- Complete setup with payments and anon policies
...
CREATE POLICY "Temp anon read payments" ON public.payments
  FOR SELECT TO anon
  USING (true);

CREATE POLICY "Temp anon insert payments" ON public.payments
  FOR INSERT TO anon
  WITH CHECK (true);

CREATE POLICY "Temp anon update payments" ON public.payments
  FOR UPDATE TO anon
  USING (true);

-- Grant sequence usage for payments
GRANT USAGE ON SEQUENCE public.payments_id_seq TO anon;
